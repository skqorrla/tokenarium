...
class FeedData:
    ...
    source: str          # "Codex" | "codex" | "gemini" | "git"
    timestamp: datetime
...
### AI Limb 공통 구조 (Codex / codex / gemini)
...
| AI | 감시 경로 | 토큰 필드 |
|---|---|---|
| Codex | `~/.Codex/projects/**/*.jsonl` | `usage.input_tokens` + `usage.output_tokens` |
...
Codex 프로젝트 식별: `-home-user-Project-myapp` 폴더명 → `rsplit("-",1)[-1]` → `"myapp"`
